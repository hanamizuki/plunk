...
describe('BounceService', () => {
  ...
  async function bounceEvent(contactId: string, createdAt: Date, data: Record<string, unknown>) {
    return prisma.event.create({
      data: {projectId, contactId, name: 'email.bounce', data, createdAt},
    });
  }
  ...
  describe('isUserNotFoundBounce', () => {
    ...
    it('falls back to the diagnostic code when the DSN status is missing', () => {
      const bounce: SesBounce = {
        bounceType: 'Permanent',
        bouncedRecipients: [{emailAddress: 'a@privaterelay.appleid.com', diagnosticCode: 'smtp; 550 5.1.1 user not found'}],
      };
  ...
  describe('evaluateRelayStrike', () => {
    ...
    it('does not escalate on same-day repeats', async () => {
      ...
      await bounceEvent(relay.id, new Date('2026-08-27T09:00:00Z'), {bounceType: 'Permanent', relayStrike: 1});
      await bounceEvent(relay.id, new Date('2026-08-27T11:00:00Z'), {bounceType: 'Permanent', relayStrike: 1});
    ...
    it('unsubscribes once the threshold is reached on distinct days', async () => {
      ...
      for (let i = 1; i < RELAY_HARD_BOUNCE_STRIKES; i++) {
        await bounceEvent(relay.id, new Date(now.getTime() - i * DAY_MS), {bounceType: 'Permanent', relayStrike: i});
      }
    ...
    it('ignores strikes outside the window and bounces recorded without a strike marker', async () => {
      const relay = await factories.createContact({projectId, email: 'abc123@privaterelay.appleid.com'});
      const now = new Date('2026-08-27T14:00:00Z');
      await bounceEvent(relay.id, new Date(now.getTime() - (RELAY_STRIKE_WINDOW_DAYS + 1) * DAY_MS), {
        bounceType: 'Permanent',
        relayStrike: 1,
      });
      // Historical hard bounce recorded before strike tracking existed — must not count
      await bounceEvent(relay.id, new Date(now.getTime() - 2 * DAY_MS), {bounceType: 'Permanent'});
  ...
  describe('isHardBounced', () => {
    it('is true for a contact unsubscribed by a permanent bounce', async () => {
      ...
      await bounceEvent(contact.id, new Date(), {bounceType: 'Permanent'});
    ...
    it('is false while the contact is subscribed, even with bounce history', async () => {
      ...
      await bounceEvent(contact.id, new Date(), {bounceType: 'Permanent', relayStrike: 1});
    ...
    it('is false for manual unsubscribes and for transient-only bounce history', async () => {
      ...
      await bounceEvent(soft.id, new Date(), {bounceType: 'Transient', transientBounce: true});
